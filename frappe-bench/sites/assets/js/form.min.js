...
$(document).on("page-change",function(){var open_form=frappe.ui.form.get_open_grid_form();open_form&&open_form.hide_form();});},show_doc:function(route){var dt=route[1],dn=route.slice(2).join("/"),me=this;if(frappe.model.new_names[dn]){dn=frappe.model.new_names[dn];frappe.set_route("Form",dt,dn);return;}
...
_f.Frm.prototype.setup=function(){var me=this;this.fields=[];this.fields_dict={};this.state_fieldname=frappe.workflow.get_state_fieldname(this.doctype);this.wrapper=this.parent;frappe.ui.make_app_page({parent:this.wrapper,single_column:this.meta.hide_toolbar});this.page=this.wrapper.page;this.layout_main=this.page.main.get(0);this.sidebar=new frappe.ui.form.Sidebar({frm:this,page:this.page});this.toolbar=new frappe.ui.form.Toolbar({frm:this,page:this.page});this.setup_print_layout();this.setup_std_layout();this.script_manager=new frappe.ui.form.ScriptManager({frm:this});this.script_manager.setup();this.watch_model_updates();if(!this.meta.hide_toolbar){this.footer=new frappe.ui.form.Footer({frm:this,parent:$('<div>').appendTo(this.page.main.parent())})}
...
_f.Frm.prototype.hide_print=function(){if(this.setup_done){this.page.set_view(this.page.previous_view_name!="print"&&this.page.previous_view_name||"main");}}
...
_f.Frm.prototype.setup_std_layout=function(){this.form_wrapper=$('<div></div>').appendTo(this.layout_main);this.inner_toolbar=$('<div class="form-inner-toolbar hide"></div>').appendTo(this.form_wrapper);this.body=$('<div></div>').appendTo(this.form_wrapper);this.meta.section_style='Simple';this.layout=new frappe.ui.form.Layout({parent:this.body,doctype:this.doctype,frm:this,});this.layout.make();this.fields_dict=this.layout.fields_dict;this.fields=this.layout.fields_list;this.dashboard=new frappe.ui.form.Dashboard({frm:this,});this.states=new frappe.ui.form.States({frm:this});}
...
frappe.ui.toolbar.recent.add(this.doctype,this.docname,1);if(this.toolbar){this.toolbar.refresh();}
...
_f.Frm.prototype.render_form=function(){if(!this.meta.istable){this.refresh_header();this.sidebar.refresh();this.script_manager.trigger("refresh");$(document).trigger('form_refresh');this.refresh_fields();if(this.cscript.is_onload){this.script_manager.trigger("onload_post_render");}
...
var validated;_f.Frm.prototype.save=function(save_action,callback,btn,on_error){btn&&$(btn).prop("disabled",true);$(document.activeElement).blur();var open_form=frappe.ui.form.get_open_grid_form();open_form&&open_form.hide_form();var me=this;setTimeout(function(){me._save(save_action,callback,btn,on_error)},100);}
...
if(save_action!="Update"){validated=true;$.when(this.script_manager.trigger("validate"),this.script_manager.trigger("before_save")).done(function(){if(!validated){if(on_error)
on_error();return;}
...
_f.Frm.prototype.add_custom_button=function(label,fn,icon,toolbar_or_class){return $('<button class="btn btn-default btn-xs" style="margin-left: 10px;">'+__(label)+'</btn>').on("click",fn).appendTo(this.inner_toolbar.removeClass("hide"))}
_f.Frm.prototype.clear_custom_buttons=function(){this.inner_toolbar.empty().addClass("hide");this.page.clear_user_actions();}
...
_f.Frm.prototype.get_docfield=function(fieldname){return frappe.meta.get_docfield(this.doctype,fieldname,this.docname);}
...
this.star_icon=this.page.add_action_icon("icon-star",function(){frappe.ui.toggle_star(me.star_icon,me.frm.doctype,me.frm.doc.name);}).removeClass("text-muted").find(".icon-star").addClass("star-action");if(frappe.model.can_email(null,me.frm)){this.page.add_menu_item(__("Email"),function(){me.frm.email_doc();},true);}
...
frappe.ui.form.handlers[doctype][fieldname].push(handler);cur_frm.events[fieldname]=handler;}
...
var state=this.get_state();if(state){this.show_actions(state);}},show_actions:function(state){var added=false,me=this;this.frm.page.clear_actions_menu();$.each(frappe.workflow.get_transitions(this.frm.doctype,state),function(i,d){if(in_list(user_roles,d.allowed)){added=true;me.frm.page.add_action_item(__(d.action),function(){var action=d.action;var doc_before_action=copy_dict(me.frm.doc);var next_state=frappe.workflow.get_next_state(me.frm.doctype,me.frm.doc[me.state_fieldname],action);me.frm.doc[me.state_fieldname]=next_state;var new_state=frappe.workflow.get_document_state(me.frm.doctype,next_state);var new_docstatus=cint(new_state.doc_status);console.log(new_state);if(new_state.update_field){me.frm.set_value(new_state.update_field,new_state.update_value);}
...
+"&no_letterhead="+(me.with_letterhead()?"0":"1"));if(!w){msgprint(__("Please enable pop-ups"));return;}}});this.wrapper.find(".btn-print-edit").on("click",function(){var print_format=me.get_print_format();if(print_format&&print_format.name){if(print_format.print_format_builder){frappe.route_options=print_format;frappe.set_route("print-format-builder");}else{frappe.set_route("Form","Print Format",print_format.name);}}else{frappe.prompt({fieldname:"print_format_name",fieldtype:"Data",reqd:1,label:"New Print Format Name"},function(data){frappe.route_options={make_new:true,doctype:me.frm.doctype,name:data.print_format_name};frappe.set_route("print-format-builder");},__("New Custom Print Format"),__("Start"));}});},preview:function(){var me=this;this.get_print_html(function(html){me.wrapper.find(".print-format").html(html);});},printit:function(){this.new_page_preview(true);},new_page_preview:function(printit){var me=this;this.get_print_html(function(html){var w=window.open("/print?"
+"doctype="+encodeURIComponent(me.frm.doc.doctype)
+"&name="+encodeURIComponent(me.frm.doc.name)
...
this.user_actions.find(".user-action-row").remove();},});;frappe.provide("frappe.ui.form");frappe.ui.form.Share=Class.extend({init:function(opts){$.extend(this,opts);},refresh:function(){var me=this;this.parent.empty();var shared=$.map(this.shared||this.frm.get_docinfo().shared,function(s){return s?s.user:null;});for(var i=0;i<shared.length;i++){var user_info=frappe.user_info(shared[i])
...
this.shared=shared;var d=this.dialog;$(d.body).empty();$(frappe.render_template("set_sharing",{frm:this.frm,shared:this.shared})).appendTo(d.body);if(frappe.model.can_share(null,this.frm)){this.make_user_input();this.add_share_button();this.set_edit_share_events();}else{$(d.body).find(".edit-share").prop("disabled",true);}},make_user_input:function(){this.dialog.share_with=frappe.ui.form.make_control({parent:$(this.dialog.body).find(".input-wrapper-add-share"),df:{fieldtype:"Link",label:__("Share With"),fieldname:"share_with",options:"User",filters:{"user_type":"System User","name":["!=",user]}},only_input:true,render_input:true});},add_share_button:function(){var me=this,d=this.dialog;$(d.body).find(".btn-add-share").on("click",function(){var user=d.share_with.get_value();if(!user){return;}
frappe.call({method:"frappe.share.add",args:{doctype:me.frm.doctype,name:me.frm.doc.name,user:user,read:$(d.body).find(".add-share-read").prop("checked")?1:0,write:$(d.body).find(".add-share-write").prop("checked")?1:0,share:$(d.body).find(".add-share-share").prop("checked")?1:0},callback:function(r){$.each(me.shared,function(i,s){if(s&&s.user===r.message.user){delete me.shared[i];}})
me.dirty=true;me.shared.push(r.message);me.render_shared();me.frm.shared.refresh();}});});},set_edit_share_events:function(){var me=this,d=this.dialog;$(d.body).find(".edit-share").on("click",function(){var user=$(this).parents(".shared-user:first").attr("data-user"),value=$(this).prop("checked")?1:0,property=$(this).attr("name");frappe.call({method:"frappe.share.set_permission",args:{doctype:me.frm.doctype,name:me.frm.doc.name,user:user,permission_to:property,value:value},callback:function(r){$.each(me.shared,function(i,s){if(s&&s.user===user){if(!r.message){delete me.shared[i];}else{me.shared[i]=$.extend(s,r.message);}
return false;}});me.dirty=true;me.render_shared();me.frm.shared.refresh();}});});},});;frappe.templates["set_sharing"] = '<div class="padding"> {% if(!shared.length) { %} <p class="text-muted">{%= __("Not shared with anyone yet.") %}</p> {% } else { %} <div class="row"> <div class="col-xs-6"><h6>{%= __("User") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Read") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Write") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Share") %}</h6></div> </div> {% for (var i=0, l=shared.length; i < l; i++) { var s = shared[i]; %} {% if(s) { %} <div class="row shared-user" data-user="{%= s.user %}" data-name="{%= s.name %}"> <div class="col-xs-6">{%= s.user %}</div> <div class="col-xs-2"><input type="checkbox" name="read" {% if(cint(s.read)) { %}checked{% } %} class="edit-share"></div> <div class="col-xs-2"><input type="checkbox" name="write" {% if(cint(s.write)) { %}checked{% } %} class="edit-share"></div> <div class="col-xs-2"><input type="checkbox" name="share" {% if(cint(s.share)) { %}checked{% } %} class="edit-share"></div> </div> {% } %} {% } %} {% } %} {% if(frappe.model.can_share(null, frm)) { %} <hr> <div class="row"> <div class="col-xs-6"><h6>{%= __("Share this document with") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Read") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Write") %}</h6></div> <div class="col-xs-2"><h6>{%= __("Can Share") %}</h6></div> </div> <div class="row"> <div class="col-xs-6 input-wrapper-add-share"></div> <div class="col-xs-2"><input type="checkbox" class="add-share-read" name="read"></div> <div class="col-xs-2"><input type="checkbox" class="add-share-write" name="write"></div> <div class="col-xs-2"><input type="checkbox" class="add-share-share" name="share"></div> </div> <p> <button class="btn btn-primary btn-add-share">{%= __("Add") %}</button> </p> {% } %} </div> ';
frappe.templates["form_sidebar"] = '<ul class="list-unstyled sidebar-menu user-actions hide"> <li class="divider"></li> </ul> <ul class="list-unstyled sidebar-menu"> <li class="divider"></li> <li style="position: relative;"> <a class="strong sidebar-comments badge-hover"> <span>{%= __("Comments") %}</span> <span class="badge n-comments">0</span> </a> </li> </ul> <ul class="list-unstyled sidebar-menu form-assignments"> <li class="divider"></li> <li class="h6 assigned-to-label">{%= __("Assigned To") %}</li> <li><a class="strong add-assignment">{%= __("Assign") %} <i class="octicon octicon-plus" style="margin-left: 2px;"></i></a></li> </ul> <ul class="list-unstyled sidebar-menu form-attachments"> <li class="divider"></li> <li class="h6 attachments-label">{%= __("Attachments") %}</li> <li class="divider"></li> <li><a class="strong add-attachment">{%= __("Attach File") %} <i class="octicon octicon-plus" style="margin-left: 2px;"></i></li></a> </ul> <ul class="list-unstyled sidebar-menu"> <li class="divider"></li> <li class="h6 tags-label">{%= __("Tags") %}</li> <li class="form-tags"> <div class="tag-area"></div> <div class="clearfix"></div> </li> </ul> <ul class="list-unstyled sidebar-menu"> <li class="divider"></li> <li class="h6 shared-with-label">{%= __("Shared With") %}</li> <li class="form-shared"></li> </ul> <ul class="list-unstyled sidebar-menu text-muted"> <li class="modified-by"></li> <li class="created-by"></li> </ul> ';
...
frappe.templates["timeline_item"] = '<div class="media timeline-item" data-name="{%= data.name %}"> <span class="pull-left avatar avatar-medium"> <img class="media-object" src="{%= data.image %}"> </span> <div class="pull-left media-body" style="max-width: calc(100% - 51px);"> <div> <div class="pull-right"> <span class="small text-muted"> {%= data.delete %} </span> </div> {% if(data.doctype=="Communication" || data.comment_type=="Comment") { %} <h6> <i class="{%= data.icon %} icon-fixed-width"></i> <span title="{%= data.comment_by %}">{%= data.fullname %}</span> <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> {% if(data.doctype=="Communication") { %} &ndash; <a href="#Form/{%= data.doctype %}/{%= data.name %}" class="text-muted">{%= __("Details") %}</a> {% } %} </h6> <div class="reply"> <div> {%= data.comment_html %} </div> </div> {% } else if(in_list(["Assignment Completed", "Assigned", "Shared", "Unshared"], data.comment_type)) { %} <h6> <i class="{%= data.icon %} icon-fixed-width"></i> {%= data.comment %} <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> </h6> {% } else { %} <h6> <i class="{%= data.icon %} icon-fixed-width"></i> <span title="{%= data.comment_by %}">{%= data.fullname %}</span> {%= data.comment %} <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> </h6> {% } %} {% if(data.attachments && data.attachments.length) { %} <div style="margin: 10px 0px"> {% $.each(data.attachments, function(i, a) { %} <div class="text-ellipsis"> <a href="{%= a %}" class="text-muted small" target="_blank"> <i class="icon-paperclip"></i> {%= a.split("/").slice(-1)[0] %} </a> </div> {% }); %} </div> {% } %} </div> </div> </div> ';
...
var me=this;var $attach=$(repl('<li class="attachment-row">\
   ...
   </li>',{file_name:file_name,file_url:file_url})).insertAfter(this.attachments_label.addClass("has-attachments"));var $close=$attach.find(".close").data("fileid",fileid).click(function(){var remove_btn=this;frappe.confirm(__("Are you sure you want to delete the attachment?"),function(){me.remove_attachment($(remove_btn).data("fileid"))});return false});if(!frappe.model.can_write(this.frm.doctype,this.frm.name)){$close.remove();}},get_file_url:function(attachment){var file_url=attachment.file_url;if(!file_url){if(attachment.file_name.indexOf('files/')===0){file_url='/'+attachment.file_name;}
...
if(!frappe.utils.is_html(c.comment)){c.comment_html=frappe.markdown(__(c.comment));}else{c.comment_html=c.comment;c.comment_html=frappe.utils.strip_whitespace(c.comment_html);}}
if(in_list(["Comment","Email","Assignment Completed"],c.comment_type)){c.padding="padding-left: 8px;";}else if(in_list(["Created"],c.comment_type)){c.padding="padding-left: 9px;";}else{c.padding="";}
$(frappe.render_template("timeline_item",{data:c})).appendTo(me.list).on("click",".close",function(){var name=$(this).parents(".timeline-item:first").attr("data-name");me.delete_comment(name);return false;});if(c.comment_type==="Email"){this.last_type=c.comment_type;}},set_icon_and_color:function(c){c.icon={"Email":"octicon octicon-mail","Chat":"octicon octicon-comment-discussion","Phone":"octicon octicon-device-mobile","SMS":"octicon octicon-comment","Created":"octicon octicon-plus","Submitted":"octicon octicon-lock","Cancelled":"octicon octicon-x","Assigned":"octicon octicon-person","Assignment Completed":"octicon octicon-check","Comment":"octicon octicon-comment-discussion","Workflow":"octicon octicon-git-branch","Label":"octicon octicon-tag","Attachment":"octicon octicon-cloud-upload","Attachment Removed":"octicon octicon-trashcan","Shared":"octicon octicon-eye","Unshared":"octicon octicon-circle-slash"}[c.comment_type]
